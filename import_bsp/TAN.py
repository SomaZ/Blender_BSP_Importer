...
class surface_factory:
    ...
    def __init__(self, objects, individual, material_mapping = None, material_merge = True):
        ...
        for obj in objects:
            if material_merge:
                if len(obj.data.materials) == 0:
                    surfaces["NoShader"] = [surface_descriptor("NoShader", "material"+str(material_count))]
                    if material_mapping != None:
                        material_mapping["material"+str(material_count)] = "NoShader"
                    material_count += 1
                for mat in obj.data.materials:
                    mat_name = mat.name.split(".")[0]
                    if mat_name not in surfaces:
                        surfaces[mat_name] = [surface_descriptor(mat_name, "material"+str(material_count))]
                        if material_mapping != None:
                ...
                surfaces[obj.name] = [surface_descriptor(mat_name, "material"+str(material_count))]
                if material_mapping != None:
                    material_mapping["material"+str(material_count)] = mat_name
                material_count += 1
            ...
            for triangle in mesh.loop_triangles:
                if material_merge:
                    ...
                    if mat in surfaces:
                        ...
                        if not succeeded:
                            new_surface_descr = surface_descriptor(mat, "material"+str(material_count))
                            material_count += 1
...
class TAN:
    ...
    class surface:
        size = INT + STRING + INT + INT + INT + INT + INT + INT + INT + INT + INT
        ...
        def __init__(self, array):
            ...
            self.triangles =    tan_array(self.triangle,[self.off_tris, self.n_tris])
            self.vertices =     tan_array(self.vertex,[self.off_verts, self.n_verts])
            self.tcs =          tan_array(self.tc,[self.off_tcs, self.n_verts])
            self.collapse_map = tan_array(self.collapse_map,[self.off_collapse, self.n_verts])
            
        def get_frame_vertex_lump(self, frame):
            if frame > self.n_frames:
                return None
            return tan_array(self.vertex,[self.off_verts + frame*self.n_verts*self.vertex.size, self.n_verts])
        ...
        def from_surface_descriptor(cls, sd):
            ...
            for triangle in sd.triangles:
                ...
                surface.collapse_map.data.append(cls.collapse_map([cls.triangle(triangle).indices[0]]))
    ...
    class frame:
        size = 3*FLOAT + 3*FLOAT + 3*FLOAT + 3*FLOAT + 3*FLOAT + FLOAT + FLOAT
        ...
        def from_objects(cls, objects, individual, previous_frame = None):
            ...
            frame.scale[0] = (frame.max_bounds[0] - frame.min_bounds[0]) / 65535
            frame.scale[1] = (frame.max_bounds[1] - frame.min_bounds[1]) / 65535
            frame.scale[2] = (frame.max_bounds[2] - frame.min_bounds[2]) / 65535
...
def ImportTAN(model_name, material_mapping, import_tags = False, animations = None, per_object_import = False):
    ...
    if (not skip):
        ...
        total_time  = struct.unpack("<f", file.read(4))[0]
        total_delta = struct.unpack("<fff", file.read(12))
...
def ImportTANObject(file_path, material_mapping, import_tags, per_object_import = False):
    animations = []
    meshes = ImportTAN(file_path, material_mapping, import_tags, animations, per_object_import)
    objs = []
...
def ExportTAN(file_path, objects, frame_list, individual, material_mapping = None, material_merge = True):
    ...
    sf = surface_factory(eval_mesh_objects, individual, material_mapping, material_merge)
    if not sf.valid:
    ...
    for frame in frame_list[1:]:
        ...
        for surf_id, surf in enumerate(surface_descriptors):
            surfaces[surf_id].add_current_frame(surf, frame.scale, frame.offset)
    ...
    ofsFrames = INT + INT + STRING + INT + INT + INT + FLOAT + FLOAT + FLOAT + FLOAT + 19*INT
        ...
        for line in lines:
            ...
            if is_open == 0:
                ...
                if dict_key == "setup" and key == "surface":
                    ...
                    if not "material_mapping" in dict[dict_key]:
                    ...
                    if not shader_name.endswith(".tga"):
                        dict[dict_key]["material_mapping"][arguments[0].lower()] = shader_name
                    else:
                        dict[dict_key]["material_mapping"][arguments[0].lower()] = base_path + shader_name
                dict[dict_key][key] = value
    return dict

# TODO: parse scale and other fields
def ImportTIK(file_path, zoffset, import_tags = False, animations = None, per_object_import = False):
    ...
    if not model_path.endswith(".tan"):
        print("Only .tan models are currently supported. Tried loading: " + dict["setup"]["path"] + dict["animations"]["idle"])
    return ImportTAN(model_path, material_mapping, import_tags, animations, per_object_import)

def ImportTIKObject(file_path, import_tags, per_object_import = False):
    dict = TIK_TO_DICT(file_path)
    material_mapping = None
    current_path = ""
    model_to_load = ""
    ...
    if not model_path.endswith(".tan"):
        print("Only .tan models are currently supported. Tried loading: " + dict["setup"]["path"] + dict["animations"]["idle"])
    return ImportTANObject(model_path, material_mapping, import_tags, per_object_import)

def ExportTIK_TAN(file_path, subpath, objects, frame_list, individual, material_merge = True):
    return_status = [False, "Unknown Error"]
    ...
    try:
        ...
        return_status = ExportTAN(tik_path+subpath+tan_name, objects, frame_list, individual, material_mapping, material_merge)
    except:
    ...
    try:
        ...
        for material in material_mapping:
            f.write("\tsurface " + str(material) + " shader " + str(material_mapping[material]) + "\n")
        f.write("}\n\n")
        f.write("init\n{\n")
        f.write("\tserver\n")
        f.write("\t{\n")
