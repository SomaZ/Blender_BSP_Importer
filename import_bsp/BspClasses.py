    ...
    def readFrom(self, file):
        ...
        for i in range(self.count):
            self.data.append(self.data_class(struct.unpack(
                self.data_class.encoding, file.read(self.data_class.size))))
...
class leaf:
    ...
    def __init__(self, array):
        ...
        # looks like ef2 has number of leaf faces first
...
class vertex_rbsp:
    ...
    def __init__(self, array):
        ...
        self.color1 = [float(array[16]/255.0), float(array[17]/255.0),
                       float(array[18]/255.0), float(array[19]/255.0)]
        self.color2 = [float(array[20]/255.0), float(array[21]/255.0),
                       float(array[22]/255.0), float(array[23]/255.0)]
        self.color3 = [float(array[24]/255.0), float(array[25]/255.0),
                       float(array[26]/255.0), float(array[27]/255.0)]
        self.color4 = [float(array[28]/255.0), float(array[29]/255.0),
                       float(array[30]/255.0), float(array[31]/255.0)]
...
class vertex_ibsp:
    ...
    def __init__(self, array):
        ...
        self.color1 = [float(array[10]/255.0), float(array[11]/255.0),
                       float(array[12]/255.0), float(array[13]/255.0)]
...
class vertex_ef2:
    ...
    def __init__(self, array):
        ...
        self.color1 = [float(array[8]/255.0), float(array[9]/255.0),
                       float(array[10]/255.0), float(array[11]/255.0)]
...
# rbsp
class face_rbsp:
    size = INT + INT + INT + INT + INT + INT + INT + 4*UBYTE + 4*UBYTE + \
        4*INT + 4*INT + 4*INT + INT + INT + 3*FLOAT + 3*3*FLOAT + INT + INT
    ...
    def __init__(self, array):
        ...
        self.lm_vecs = [array[32], array[33], array[34], array[35],
...
class face_ibsp:
    ...
    def __init__(self, array):
        ...
        self.lm_vecs = [array[15], array[16], array[17], array[18],
                        array[19], array[20], array[21], array[22], array[23]]
    ...
    def __init__(self, file_name):
        ...
        if self.valid:
            for lump in self.lumps:
                self.lumps[lump].set_offset_size(
                    struct.unpack("<ii", file.read(8)))
            for lump in self.lumps:
                self.lumps[lump].readFrom(file)
                print(lump + "offset " + str(
                    self.lumps[lump].offset) + "\t size " + str(
                    self.lumps[lump].size) + "\t count " + str(
                    self.lumps[lump].count))
        else:
            print("Could not import the bsp. Bsp Version: " +
                  str(magic_nr) + " " + str(version_nr))
...
def ImportBSP(VFS, import_settings):
    ...
    if bsp.valid:
        ...
        if import_settings.preset == "BRUSHES":
            ...
            if collection is not None:
                QuakeShader.build_quake_shaders(
                    import_settings, bpy.data.collections["Brushes"].objects)
                ...
                for obj in bpy.data.collections["Brushes"].objects:
                    ...
                    for m in obj.material_slots:
                        ...
                        if "Sky" in m.material.node_tree.nodes:
                            delete_obj = True
                            break
                        if "Transparent" not in m.material.node_tree.nodes:
                            delete_obj = False
                    if delete_obj:
        ...
        import_settings.log.append(
            "took:" + str(perf_counter() - time_start) + " seconds")
        ...
        for model_index in range(int(bsp.lumps["models"].count)):
            ...
            mesh.uv_layers["UVMap"].data.foreach_set(
                "uv", unpack_list(unpack_list(model.face_tcs)))

            mesh.uv_layers.new(do_init=False, name="LightmapUV")
            mesh.uv_layers["LightmapUV"].data.foreach_set(
                "uv", unpack_list(unpack_list(model.face_lm1_tcs)))

            mesh.vertex_colors.new(name="Color")
            mesh.vertex_colors["Color"].data.foreach_set(
                "color", unpack_list(unpack_list(model.face_vert_color)))

            if bsp.lightmaps > 1:
                mesh.uv_layers.new(do_init=False, name="LightmapUV2")
                mesh.uv_layers["LightmapUV2"].data.foreach_set(
                    "uv", unpack_list(unpack_list(model.face_lm2_tcs)))

                mesh.uv_layers.new(do_init=False, name="LightmapUV3")
                mesh.uv_layers["LightmapUV3"].data.foreach_set(
                    "uv", unpack_list(unpack_list(model.face_lm3_tcs)))

                mesh.uv_layers.new(do_init=False, name="LightmapUV4")
                mesh.uv_layers["LightmapUV4"].data.foreach_set(
                    "uv", unpack_list(unpack_list(model.face_lm4_tcs)))

                mesh.vertex_colors.new(name="Color2")
                mesh.vertex_colors["Color2"].data.foreach_set(
                    "color", unpack_list(unpack_list(model.face_vert_color2)))

                mesh.vertex_colors.new(name="Color3")
                mesh.vertex_colors["Color3"].data.foreach_set(
                    "color", unpack_list(unpack_list(model.face_vert_color3)))

                mesh.vertex_colors.new(name="Color4")
                mesh.vertex_colors["Color4"].data.foreach_set(
                    "color", unpack_list(unpack_list(model.face_vert_color4)))

            # ugly hack to get the vertex alpha.....
            mesh.vertex_colors.new(name="Alpha")
            mesh.vertex_colors["Alpha"].data.foreach_set(
                "color", unpack_list(unpack_list(model.face_vert_alpha)))

            if bpy.app.version >= (2, 92, 0):
                mesh.attributes.new(name="BSP_VERT_INDEX",
                                    type='INT', domain='POINT')
                mesh.attributes["BSP_VERT_INDEX"].data.foreach_set(
                    "value", model.vertex_bsp_indices)
                ...
                mesh.vertex_layers_int["BSP_VERT_INDEX"].data.foreach_set(
                    "value", model.vertex_bsp_indices)
        ...
        ent_list = Entities.ImportEntities(VFS, bsp, import_settings)
        ...
        import_settings.log.append(
            "BSP Lightgrid Origin: " + str(bsp.lightgrid_origin))
        import_settings.log.append(
            "BSP Lightgrid Size: " + str(bsp.lightgrid_size))
        dimensions = [1.0 / bsp.lightgrid_inverse_dim[0],
                      1.0 / bsp.lightgrid_inverse_dim[1],
                      1.0 / bsp.lightgrid_inverse_dim[2]]
        import_settings.log.append(
            "BSP Lightgrid Dimensions: " + str(dimensions))
        import_settings.log.append(
            "took:" + str(perf_counter() - time_start) + " seconds")
        ...
        for ent in ent_list:
            if ent.data.name in vertex_groups:
                for vertex_group in vertex_groups[ent.data.name]:
                    ...
                    ent.vertex_groups[vertex_group].add(
                        list(vertex_groups[ent.data.name][vertex_group]),
                        1.0,
                        "ADD")

        vg = ent_list[0].vertex_groups.get("Decals")
        if vg is not None:
            modifier = ent_list[0].modifiers.new(
                "polygonOffset", type="DISPLACE")
