...
def pack_lightmaps(bsp, lightmap_lump_name, import_settings):
    ...
    try:
        lm_filenames = [Image.remove_file_extension(
    ...
    if not import_settings.mixed_lightmaps:
        ...
        for image in ext_images:
            ...
            if lightmap_size[0] != 0 and (
                (lightmap_size[0] != image.size[0]) or (
                    lightmap_size[1] != image.size[1])):
                use_internal_lightmaps = True
                break
    ...
    for i in range(6):
        if (n_lightmaps > int(max_lightmaps)):
            ...
            num_columns = import_settings.packed_lightmap_size[0] / \
                lightmap_size[0]
            num_rows = import_settings.packed_lightmap_size[1] / \
                lightmap_size[1]
            max_lightmaps = num_columns * num_rows
        else:
            import_settings.log.append(
                "Found best packed lightmap size: " + str(
    ...
    for pixel in range(packed_lm_size[0]*packed_lm_size[1]):
        ...
        if (lightmap_id > n_lightmaps-1) or (lightmap_id < 0):
            ...
            pixels[4 * pixel + 0] = (
                float(lightmaps[lightmap_id][pixel_id*color_components + 0]) /
                color_scale)
            pixels[4 * pixel + 1] = (
                float(lightmaps[lightmap_id][pixel_id*color_components + 1]) /
                color_scale)
            pixels[4 * pixel + 2] = (
                float(lightmaps[lightmap_id][pixel_id*color_components + 2]) /
                color_scale)
...
def pack_lightgrid(bsp):
    ...
    num_elements = int(lightgrid_dimensions[0] *
                       lightgrid_dimensions[1] *
                       lightgrid_dimensions[2])
    num_elements_bsp = bsp.lumps["lightgridarray"].count if (
        bsp.use_lightgridarray) else bsp.lumps["lightgrid"].count
    ...
    if num_elements == num_elements_bsp:
        for pixel in range(num_elements):
            ...
            lat = ((bsp.lumps["lightgrid"].data[index].lat_long[0]/255.0) *
                   2.0 * pi)
            long = ((bsp.lumps["lightgrid"].data[index].lat_long[1]/255.0) *
                    2.0 * pi)
...
class blender_model_data:
    ...
    def parse_bsp_surface(model, bsp, face, shaders_lump, import_settings):
        ...
        for i in range(int(face.n_indexes / 3)):
            ...
            index_0 = face.vertex + \
                bsp.lumps["drawindexes"].data[index + 0].offset
            index_1 = face.vertex + \
                bsp.lumps["drawindexes"].data[index + 2].offset
            index_2 = face.vertex + \
                bsp.lumps["drawindexes"].data[index + 1].offset
            ...
            indices = (model.index_mapping[index_0],
                       model.index_mapping[index_1],
                       model.index_mapping[index_2])
            tcs = []
            lm1_tcs = []
            colors = []
            ...
            for v_id, face_index in enumerate([index_0, index_1, index_2]):
                ...
                if model.lightmaps > 1:
                    lm2_tcs.append(
                        pack_lm_tc(drawverts_lump[face_index].lm2coord,
                                   face.lm_indexes[1],
                                   model.lightmap_size,
                                   import_settings))
                    lm3_tcs.append(
                        pack_lm_tc(drawverts_lump[face_index].lm3coord,
                                   face.lm_indexes[2],
                                   model.lightmap_size,
                                   import_settings))
                    lm4_tcs.append(
                        pack_lm_tc(drawverts_lump[face_index].lm4coord,
                                   face.lm_indexes[3],
                                   model.lightmap_size,
                                   import_settings))
                ...
                alphas.append([drawverts_lump[face_index].color1[3],
                               drawverts_lump[face_index].color1[3],
                               drawverts_lump[face_index].color1[3],
                               drawverts_lump[face_index].color1[3]])
                index += 1
            ...
            model.face_materials.append(
                model.material_names.index(material_name))
    ...
    def parse_patch_surface(model, bsp, face, shaders_lump, import_settings):
        ...
        ctrlPoints = [[0 for x in range(MAX_GRID_SIZE)]
                      for y in range(MAX_GRID_SIZE)]
        indicesPoints = [[0 for x in range(MAX_GRID_SIZE)]
                         for y in range(MAX_GRID_SIZE)]
        bspPoints = [[-1 for x in range(MAX_GRID_SIZE)]
                     for y in range(MAX_GRID_SIZE)]
        for i in range(face.patch_width):
            for j in range(face.patch_height):
                ...
                indicesPoints[j][i] = model.index_mapping[
                    face.vertex + j * face.patch_width + i]
            ...
            for subd in range(import_settings.subdivisions):
                ...
                for i in range(width//2):
                    ...
                    for j in range(height+1):
                        prev = lerpVertices(
                            ctrlPoints[j][pos_w],
                            ctrlPoints[j][pos_w+1],
                            model.vertex_class,
                            model.lightmaps)
                        next = lerpVertices(
                            ctrlPoints[j][pos_w+1],
                            ctrlPoints[j][pos_w+2],
                            model.vertex_class,
                            model.lightmaps)
                        midl = lerpVertices(
                            prev,
                ...
                for j in range(height//2):
                    ...
                    for i in range(width+1):
                        prev = lerpVertices(
                            ctrlPoints[pos_h][i],
                            ctrlPoints[pos_h+1][i],
                            model.vertex_class,
                            model.lightmaps)
                        next = lerpVertices(
                            ctrlPoints[pos_h+1][i],
                            ctrlPoints[pos_h+2][i],
                            model.vertex_class,
                            model.lightmaps)
                        midl = lerpVertices(
                            prev,
        ...
        for patch_face_index in range(width*height + height - 1):
            ...
            model.face_lm1_tcs.append([
                pack_lm_tc(patch[i1].lm1coord,
                           face.lm_indexes[0],
                           model.lightmap_size,
                           import_settings,
                           0,
                           bsp.filled_vert_map_verts),
                pack_lm_tc(patch[i2].lm1coord,
                           face.lm_indexes[0],
                           model.lightmap_size,
                           import_settings,
                           1,
                           bsp.filled_vert_map_verts),
                pack_lm_tc(patch[i3].lm1coord,
                           face.lm_indexes[0],
                           model.lightmap_size,
                           import_settings,
                           2,
                           bsp.filled_vert_map_verts),
                pack_lm_tc(patch[i4].lm1coord,
                           face.lm_indexes[0],
                           model.lightmap_size,
                           import_settings,
                           3,
            ...
            if model.lightmaps > 1:
                model.face_lm2_tcs.append([
                    pack_lm_tc(patch[i1].lm2coord, face.lm_indexes[1],
                               model.lightmap_size, import_settings),
                    pack_lm_tc(patch[i2].lm2coord, face.lm_indexes[1],
                               model.lightmap_size, import_settings),
                    pack_lm_tc(patch[i3].lm2coord, face.lm_indexes[1],
                               model.lightmap_size, import_settings),
                    pack_lm_tc(patch[i4].lm2coord, face.lm_indexes[1],
                               model.lightmap_size, import_settings)
                ])

                model.face_lm3_tcs.append([
                    pack_lm_tc(patch[i1].lm3coord, face.lm_indexes[2],
                               model.lightmap_size, import_settings),
                    pack_lm_tc(patch[i2].lm3coord, face.lm_indexes[2],
                               model.lightmap_size, import_settings),
                    pack_lm_tc(patch[i3].lm3coord, face.lm_indexes[2],
                               model.lightmap_size, import_settings),
                    pack_lm_tc(patch[i4].lm3coord, face.lm_indexes[2],
                               model.lightmap_size, import_settings)
                ])

                model.face_lm4_tcs.append([
                    pack_lm_tc(patch[i1].lm4coord, face.lm_indexes[3],
                               model.lightmap_size, import_settings),
                    pack_lm_tc(patch[i2].lm4coord, face.lm_indexes[3],
                               model.lightmap_size, import_settings),
                    pack_lm_tc(patch[i3].lm4coord, face.lm_indexes[3],
                               model.lightmap_size, import_settings),
                    pack_lm_tc(patch[i4].lm4coord, face.lm_indexes[3],
                               model.lightmap_size, import_settings)
                ])

                model.face_vert_color2.append(
                    [patch[i1].color2,
                     patch[i2].color2,
                     patch[i3].color2,
                     patch[i4].color2])
                model.face_vert_color3.append(
                    [patch[i1].color3,
                     patch[i2].color3,
                     patch[i3].color3,
                     patch[i4].color3])
                model.face_vert_color4.append(
                    [patch[i1].color4,
                     patch[i2].color4,
                     patch[i3].color4,
                     patch[i4].color4])
            ...
            alphas = []
            alphas.append([patch[i1].color1[3], patch[i1].color1[3],
                          patch[i1].color1[3], patch[i1].color1[3]])
            alphas.append([patch[i2].color1[3], patch[i2].color1[3],
                          patch[i2].color1[3], patch[i2].color1[3]])
            alphas.append([patch[i3].color1[3], patch[i3].color1[3],
                          patch[i3].color1[3], patch[i3].color1[3]])
            alphas.append([patch[i4].color1[3], patch[i4].color1[3],
                          patch[i4].color1[3], patch[i4].color1[3]])
    ...
    def parse_brush(model, bsp, brush_id, import_settings):
        ...
        for side in range(brush.n_brushsides):
            ...
            shader = (bsp.lumps["shaders"].data[brushside.texture].name +
                      ".brush")
        ...
        for plane in planes:
            ...
            vert_dict = bmesh.ops.bisect_plane(
                bm,
                geom=geom,
                dist=0.1,
                plane_co=plane[0],
                plane_no=plane[1],
                clear_outer=True)
            bm_faces = bmesh.ops.contextual_create(
                bm,
    ...
    def get_bsp_model(model, bsp, id, import_settings):
        ...
        model.index_mapping = [-2 for i in range(
        ...
        if import_settings.preset == "BRUSHES":
            ...
            for index in range(current_model.n_faces):
                ...
                if (face.type == 1 or face.type == 3 or face.type == 5):
                    model.parse_bsp_surface(
                        bsp, face, bsp.lumps["shaders"].data, import_settings)
                # patches
                if (face.type == 2):
                    model.parse_patch_surface(
                        bsp, face, bsp.lumps["shaders"].data, import_settings)
    ...
    def fill_bsp_data(model, bsp, import_settings):
        ...
        for face_instance in bsp.lumps["surfaces"].data:
            # surface or mesh
            if (face_instance.type == 1 or (
                    face_instance.type == 3 or face_instance.type == 5)):
                model.parse_bsp_surface(
                    bsp.lumps["drawverts"].data,
                    face_instance,
                    bsp.lumps["shaders"].data,
                    import_settings)
